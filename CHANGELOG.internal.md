...
### Changed
- Bumped `@openai/codex-sdk` from `0.125.0` to `^0.137.0` in `cyrus-codex-runner`. The vendored CLI binary moved from `vendor/<triple>/codex/codex` to `vendor/<triple>/bin/codex`, so `resolveCodexBinary` now probes both layouts. Codex 0.128 (`#19449`) removed the inline `SandboxPolicy.readOnlyAccess` read-restriction mechanism and migrated it to named **permission profiles**; the per-thread sandbox was reworked onto that model (see the Codex sandbox entry below). Streaming/steer, startup-gap follow-ups, cross-process `thread/resume`, and full sandbox enforcement (write confinement + read restriction) were all re-validated end-to-end against the 0.137 binary.
