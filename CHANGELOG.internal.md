...
### Changed
- Codex app-server execution now uses one shared app-server process per Cyrus process, with each Codex session opening its own app-server thread over that shared JSON-RPC connection. Notifications are routed by `threadId` so concurrent sessions stay isolated without spawning one app-server process per session.
