...
### Added
- On Cyrus cloud, each Bash command the agent runs is now executed under a per-command memory budget (enforced via a Linux cgroup v2 limit) so a single runaway command is stopped on its own without taking down the whole session; if a command is killed for exceeding its budget, the event is reported back to the control plane. When a run hits this limit, its final summary now ends with a brief note pointing to the billing page so you can upgrade to more compute. This is cloud-only and has no effect on self-host or community installs. ([CYPACK-1274](https://linear.app/ceedar/issue/CYPACK-1274), [#1280](https://github.com/cyrusagents/cyrus/pull/1280))
